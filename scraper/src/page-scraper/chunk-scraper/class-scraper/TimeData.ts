import { ClassWarnings, Time } from "../../../scraper-helpers/interfaces";
import { getDays } from "./time-helpers/GetDays";
import { getTime } from "./time-helpers/GetTime";
import { getLocation } from "./time-helpers/GetLocation";
import { getWeeks } from "./time-helpers/GetWeeks";
import { getInstructor } from "./time-helpers/GetInstructor";
 ...
 */
const getTimeData = ({ data, index, classID, term }: GetTimeDataParams): GetTimeDataReturn => {
  ...
  while (index < data.length) {
    ...
    for (const day of days) {
      const timeData: Time = { day, time, weeks };
      if (location) {
