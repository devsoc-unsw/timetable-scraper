...
import { ClassWarnings, WarningTag } from "../../../../scraper-helpers/interfaces";
...
const getCourseEnrolment = ({
  ...
  // (Strict requirement)
  if (!courseEnrolment || !(courseEnrolment.enrolments >= 0 && courseEnrolment.capacity > 0)) {
    // Lax requirement
    if (!courseEnrolment || !(courseEnrolment.enrolments >= 0 && courseEnrolment.capacity >= 0)) {
      console.error(
        new Error(
          `Invalid Course Enrolment: ${courseEnrolment.enrolments} ${courseEnrolment.capacity}`,
        ),
      );
