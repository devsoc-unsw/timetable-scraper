...
const getStartDateFreerooms = (req: express.Request, res: express.Response) => {
    try {
        ...
        const termStartDate =
            data.timetableData[currTerm][FIRST_COURSE]["classes"][0]["termDates"]["start"];
        ...
        if (timeDelta >= 0) {
            ...
            const oldTermStartDate =
                data.timetableData[termArray[dummyTerm]][FIRST_COURSE]["classes"][0]["termDates"][
                    "start"
                ];
...
const getStartDateNotangles = (req: express.Request, res: express.Response) => {
    try {
        ...
        // of the term.
        const termStartDate =
            data.timetableData[term()][FIRST_COURSE]["classes"][0]["termDates"]["start"];
        res.send(termStartDate);
    } catch (e) {
        res.status(400).send("Error");
    }
